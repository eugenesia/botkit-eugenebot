...
function Botkit(configuration) {
    ...
    botkit.middleware.hears.use(function(bot, message, triggers, next) {
        // anything to trigger?
        if (!triggers || !triggers.length)  { next(); return; }

        // already heard?
        if (message.heard) { next(); return; }

        for (var t = 0; t < triggers.length; t++) {
            var trigger = triggers[t];
            if (typeof(trigger.pattern) == 'string') {
                if (message.text) {
                    if (match = message.text.match(new RegExp(trigger.pattern, 'i'))) {
                        message.match = match;
                        message.heard = true;
                        trigger.callback.apply(botkit, [bot, message]);
                        break;
                    }
                }
            }
    ...
    botkit.hears = function(triggers, events, cb) {
        ...
        for (var e = 0; e < events.length; e++) {
            var ev = events[e];
            if (!botkit.listeningFor[ev]) {
                botkit.listeningFor[ev] = [];
            }

            for (var t = 0; t < triggers.length; t++) {

                botkit.listeningFor[ev].push(
                    {
                        pattern: triggers[t],
                        callback: cb,
                    }
    ...
    botkit.oldhears = function(keywords, events, cb) {
    ...
    botkit.trigger = function(event, data) {
        ...
        // first, let's see if any of the special "hears" listeners are waiting for this event...
        // only the first matching trigger will be fired.
        var bot = data[0];
        var message = data[1];
        if (
            (botkit.listeningFor[event] && botkit.listeningFor[event].length) ||
            (botkit.events[event] && botkit.events[event].length)
        ) {
            botkit.middleware.hears.run(bot, message,
                botkit.listeningFor[event] || [],
                function(err, bot, message, triggers) {
                if ((botkit.events[event] && botkit.events[event].length)) {
                    for (var e = 0; e < botkit.events[event].length; e++) {
                        var res = botkit.events[event][e].apply(botkit, data);
                        if (res === false) {
                            break;
                        }
                    }
